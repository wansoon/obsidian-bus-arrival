...
이 플러그인은 아직 Obsidian 공식 커뮤니티 플러그인 목록에 등록되지 않았기 때문에 Obsidian의 `탐색` 화면에서 직접 검색할 수 없습니다. **BRAT**은 공식 등록 전의 플러그인을 공개 GitHub 저장소에서 설치하고 이후 Release 업데이트도 받을 수 있게 해주는 커뮤니티 플러그인입니다.

BRAT을 사용하면 `main.js`, `manifest.json`, `styles.css`를 사용자가 직접 복사할 필요가 없습니다. 향후 Bus Arrival이 Obsidian 공식 목록에 등록되면 BRAT 없이 일반 커뮤니티 플러그인과 같은 방식으로 설치할 수 있습니다.
...
`stopId`에는 긴 내부 정류소 ID를 직접 찾지 않아도 됩니다. 정류장 표지판이나 버스 앱에 표시되는 `07-045` 같은 **표기 정류소 번호**를 입력하면 플러그인이 실제 경기 정류소 ID로 자동 변환합니다.

1. [경기버스정보 모바일 웹](https://m.gbis.go.kr/)을 엽니다.
2. 정류소명 또는 주변 장소명을 검색합니다.
3. 지도 위치, 운행 노선과 진행 방향을 보고 실제 탑승할 정류소를 선택합니다.
4. 정류소 상세 화면의 표기 번호를 확인합니다. 예: `07-045`
5. 확인한 번호를 코드 블록의 `stopId`에 입력합니다.
...
- 도로 맞은편에 이름이 같은 정류소가 있을 수 있으므로 **진행 방향과 운행 노선**을 함께 확인합니다.
- `stopName`은 카드에 표시할 이름이므로 사용자가 알아보기 쉽게 자유롭게 작성할 수 있습니다.
- 공공데이터 API 응답에서 확인한 실제 정류소 ID를 알고 있다면 그것을 직접 입력해도 됩니다.
...
아래 항목은 향후 확장 후보이며 구현 일정이 확정된 기능은 아닙니다. TAGO와 지역별 공공 API를 통해 기술적으로 확장할 수 있지만, 지역마다 실시간 데이터 제공 범위와 식별 체계가 달라 사전 검증이 필요합니다.

- [ ] 현재 경기버스 연동을 독립적인 데이터 제공자(provider) 구조로 분리
- [ ] 국토교통부 TAGO 기반 전국 시내버스 정류소·노선·도착정보 지원 가능 범위 조사
- [ ] 서울·부산 등 지역별 API를 이용한 데이터 보완 방식 검토
- [ ] 시외·고속버스의 터미널 시간표·도착정보를 별도 카드 문법으로 지원
- [ ] `provider: auto`, `region` 등 지역·제공자 선택 문법 설계
- [ ] API별 활용신청, 인증키와 호출 제한 안내 추가
