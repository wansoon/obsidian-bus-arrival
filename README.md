...
지도 앱에서도 버스 도착정보를 확인할 수 있지만, 자주 이용하거나 미리 계획한 정류소와 노선을 반복해서 검색하고 문서와 지도 앱을 오가야 합니다. 이 플러그인은 지도 앱을 대체하기보다 **개인의 문서 안에 필요한 도착정보만 고정해두는 것**에 초점을 둡니다.

- 자주 이용하는 정류소와 노선을 문서에 한 번 설정하면 반복 검색 없이 바로 확인
- 정류소의 전체 노선이 아닌 내가 실제로 타는 노선만 표시
- 목적지, 하차 지점, 환승 방식에 따라 노선 묶음을 별도의 카드로 구성
- 출퇴근·등하교·여행·병원 방문·정기 외출 등의 일정과 실시간 정보를 한 화면에서 확인
- 도보 이동, 환승 방법, 시간대별 팁 같은 개인 메모를 도착정보와 함께 관리
- 같은 Markdown 문서를 데스크톱·iPhone·iPad에서 사용
- 필요할 때만 수동 또는 지정 주기로 갱신하여 API 호출량 관리

즉, 지도 앱이 **장소를 검색하고 경로를 탐색하는 도구**라면 이 플러그인은 **이미 알고 있거나 계획한 이동을 개인 기록과 함께 반복해서 확인하는 도구**입니다.
...
- 문서에 지정한 노선만 필터링
- 노선별 실시간 도착 현황 표시
- 여러 정류소 카드를 활용한 이동·환승 구간별 정보 구성
